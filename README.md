# 👩‍💻 Camila Valentina Martínez Cantor

[![Java](https://img.shields.io/badge/Java-ED8B00?logo=openjdk)](#)
[![Python](https://img.shields.io/badge/Python-3.10+-3776AB?logo=python)](#)
[![Spring Boot](https://img.shields.io/badge/Spring_Boot-6DB33F?logo=springboot)](#)
[![Django](https://img.shields.io/badge/Django-092E20?logo=django)](#)
[![Flask](https://img.shields.io/badge/Flask-000000?logo=flask)](#)
[![PostgreSQL](https://img.shields.io/badge/PostgreSQL-336791?logo=postgresql)](#)
[![MongoDB](https://img.shields.io/badge/MongoDB-47A248?logo=mongodb)](#)
[![Git](https://img.shields.io/badge/Git-F05032?logo=git)](#)

> **Estudiante de últimos semestres de Ingeniería de Sistemas**  
> *Universidad El Bosque – Bogotá, Colombia*  
> *Enfoque: Desarrollo Full Stack, Arquitectura de Software e Ingeniería de Datos*

---

## 🧬 Idea del perfil

Soy una profesional en formación con capacidad para **diseñar, estructurar y desplegar servicios web integrales** utilizando Java (Spring Boot) y Python (Django, Flask).  
Aplico principios de **Programación Orientada a Objetos (POO)** y gestiono bases de datos relacionales y no relacionales, integrando frontend y backend con un enfoque sólido en **arquitectura de software, modelado de sistemas y metodologías de desarrollo**.  
Domino la documentación técnica con diagramas MER, conceptual, de casos de uso, secuencia, despliegue, componentes y actividades, y tengo experiencia aplicando tanto **metodologías ágiles (Scrum, Kanban) como en cascada**.

---

## 📂 Proyectos Destacados

| Proyecto | Descripción | Tecnologías |
|:---|:---|:---|
| **🌿 TaxoDB** | Investigación en persistencia políglota para taxonomía biológica, combinando PostgreSQL y MongoDB mediante un ETL en Python. | PostgreSQL, MongoDB, Python |
| **🧸 Toy Store** | Sistema de gestión de inventarios y ventas con arquitectura Cliente‑Servidor, integrando Django y base de datos relacional. | MySQL, Django, Python, Bootstrap |
| **🏥 Bosque Health** | Plataforma de gestión de citas médicas y exámenes, implementada con patrón MVC, DAO/DTO y desplegada en un entorno integrado. | Java, Spring Boot, PostgreSQL, PrimeFaces |

---

## 🛠️ Stack Técnico

| Categoría | Tecnologías |
|:---|:---|
| **Lenguajes** | Java, Python, SQL |
| **Backend** | Spring Boot, Django, Flask |
| **Bases de Datos** | PostgreSQL, MySQL, MongoDB |
| **Frontend** | Bootstrap, PrimeFaces, HTML/CSS |
| **Arquitectura & Despliegue** | MVC, DAO, DTO, REST APIs, integración continua y despliegue de servicios |
| **Metodologías & Procesos** | Ágiles (Scrum, Kanban), Cascada, Ciclo de Vida del Software (SDLC) |
| **Modelado & Análisis** | MER, Entidad-Relación, Conceptual, Historias de Usuario, Casos de Uso, Despliegue, Secuencia, Componentes, Actividades, Contexto |
| **Herramientas** | Git, GitHub, Postman |

---

## 🎯 Conceptos académicos aplicados

| Concepto | Aplicación en mis proyectos |
|:---|:---|
| **POO** | Código estructurado con herencia y polimorfismo en Java y Python. |
| **Arquitectura Web** | Diseño de servicios con Spring Boot, Django y Flask, separando frontend y backend. |
| **Bases de Datos** | Modelado ACID, consultas complejas, y administración de PostgreSQL y MySQL. |
| **Persistencia Políglota** | Integración SQL/NoSQL con ETL en Python (proyecto TaxoDB). |
| **Modelado de Software** | Elaboración de diagramas MER, conceptual, casos de uso, secuencia, despliegue y componentes para planificar y documentar sistemas. |
| **Metodologías Ágiles y Tradicionales** | Uso de Scrum y Cascada según necesidades del proyecto, gestionando el ciclo de vida completo. |
| **Despliegue e Integración** | Configuración de entornos de desarrollo, integración de servicios y despliegue funcional. |

---

## 🌟 Fortalezas

- Comunicación asertiva y abierta
- Trabajo en equipo colaborativo y productivo
- Proactividad y orientación a la resolución de problemas
- Aprendizaje acelerado de nuevas tecnologías
- Resiliencia frente a desafíos técnicos

---

## 🌱 Intereses Profesionales

| Área |
|:---|
| Desarrollo Full Stack |
| Arquitectura de Software |
| Ingeniería de Datos |
| Modelado y Diseño de Sistemas |
| Metodologías de Desarrollo |
| Computación en la Nube |
| Inteligencia Artificial |

---

## 📬 Contacto

📧 [cvmartinezcantor@gmail.com](mailto:cvmartinezcantor@gmail.com)  
💼 [LinkedIn: Camila Valentina Martínez Cantor](https://linkedin.com/in/tuusuario)  
🐙 [GitHub: vmartinezcantor](https://github.com/vmartinezcantor)
